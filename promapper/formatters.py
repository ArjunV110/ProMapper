...
# ── Termux (phone-friendly, one datum per line, no tables) ─────────────
def fmt_termux(results: List[HostResult], args) -> str:
    ...
    lines = [f"PROMAPPER v3.1.0  |  {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"]
    lines.append(sep)
    lines.append("")

    for idx, res in enumerate(results, 1):
        if not res.up:
            lines.append(f"{_red('●')} HOST #{idx}: {res.host} — DOWN")
            lines.append("")
            continue
        ...
        lines.append(f"{_grn('●')} {res.host}")
        lines.append(f"   IP: {res.ip}  |  {res.latency_ms:.1f}ms{tag_str}")
        if res.reverse_dns:
            lines.append(f"   DNS: {res.reverse_dns}")
        if res.mac:
            lines.append(f"   MAC: {res.mac} ({res.mac_vendor})")
        if res.os_guess:
            lines.append(f"   OS: {res.os_guess} ({res.os_accuracy:.0f}%)")
        for k in ("cdn", "cloud", "waf", "honeypot"):
            v = getattr(res, k, "")
            if v:
        ...
        if res.ports:
            for pr in sorted(res.ports, key=lambda x: x.port):
                if args.open and pr.state != "open":
                    continue
                st = _grn(pr.state) if pr.state == "open" else _yel(pr.state)
                svc = _sanitize_text(pr.service or "", 20)
                ver = _sanitize_text(pr.version or "", 12)
                lines.append(f"   Port {pr.port}/{pr.protocol.upper()} — {st}  {svc} {ver}")
                if pr.banner and args.banner:
        ...
        if res.http_tech:
            for name, cat in res.http_tech.items():
                lines.append(f"   Tech: {name} ({cat})")
        if res.http_dirs:
            for d in res.http_dirs[:10]:
                sc = _grn(str(d["status"])) if d["status"] == 200 else _yel(str(d["status"]))
                lines.append(f"   Dir: {sc} {d['path']}")
        if res.api_endpoints:
            for ep in res.api_endpoints[:10]:
                j = ' JSON' if ep['json'] else ''
                lines.append(f"   API: {ep['status']} {ep['path']}{j}")
        if res.subdomains:
            for sd in res.subdomains[:10]:
                lines.append(f"   Sub: {sd['subdomain']} -> {sd['ip']}")
        if res.traceroute:
            for hop in res.traceroute[:10]:
                ip_s = hop['ip'] if hop['ip'] != '*' else '*'
                rtt_s = f"{hop['rtt_ms']:.1f}ms" if hop['rtt_ms'] else '*'
                lines.append(f"   Hop {hop['hop']}: {ip_s}  {rtt_s}")
        if res.geo:
            gd = res.geo
            loc = f"{gd.get('city','')}, {gd.get('region','')}, {gd.get('country','')}"
            lines.append(f"   Location: {loc}")
            if gd.get('lat') is not None:
                lines.append(f"   Coord: {gd['lat']}, {gd['lon']}")
        if res.asn:
            lines.append(f"   ASN: {res.asn}  ISP: {res.isp}")
        if res.shodan and res.shodan.get("ports"):
            lines.append(f"   Shodan: {', '.join(map(str, res.shodan['ports']))}")
        if res.whois:
            for wl in res.whois.split("\n")[:4]:
                lines.append(f"   WHOIS: {_sanitize_text(wl, 60)}")
        if res.cves:
            for cve in sorted(set(res.cves)):
                lines.append(f"   {_red(cve)}")
        if res.brute_creds:
            for cred in res.brute_creds:
                lines.append(f"   Creds: {cred['user']}:{cred['password']}")
        lines.append("")

    lines.append(sep)
    up = sum(1 for r in results if r.up)
    lines.append(f"Done: {up}/{len(results)} hosts up  |  {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("")
    return "\n".join(lines)
