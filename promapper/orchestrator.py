...
def scan_host(entry: Tuple[str, str], args: argparse.Namespace) -> HostResult:
    ...
    # Stage 4: Determine if this is info-only (--geo/--asn/--whois/--shodan/--mac-vendor with no scan/detection flags)
    _info_flags = ["geo", "asn", "whois", "shodan", "mac_vendor"]
    _has_info = any(getattr(args, f, False) for f in _info_flags)
    _has_non_info = any(getattr(args, f, False) for f in [
        "service_version", "os_guess", "traceroute", "banner", "ssl_cert",
        "http_tech", "dir_bust", "subdomain_enum", "api_discovery", "brute",
        "ssh_key", "detect_waf", "detect_cdn", "detect_cloud", "detect_honeypot",
        "cve", "udp", "syn", "fin", "null", "xmas", "ack", "window", "maimon",
    ]) or args.idle_scan or bool(args.ping_only)
    _info_only = _has_info and not _has_non_info
    ...
    # Stages 13/14/15/19: Info lookups — run regardless of info-only mode
    if args.geo:
    ...
    if _info_only:
        result.up = bool(result.ip)
    elif any(pr.state in ("open", "closed", "unfiltered") for pr in port_results):
        result.up = True
